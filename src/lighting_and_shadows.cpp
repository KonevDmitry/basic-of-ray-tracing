		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
			...
			for (size_t v = 0; v < fv; v++) {
				...
				if (idx.normal_index < 0)
					...
					vertices.push_back(
						Vertex(float3{ vx,vy,vz }, float3{ nx,ny,nz }));
				}
			}
			index_offset += fv;
	...
	for (auto& light : lights) {
		...
		if (fabs(t - to_light_length) < 0.001f) {
			...
			float3 specular_direction =
				from_light_ray.direction - 2.f * dot(from_light_ray.direction, normal) * normal;
			payload.color +=
				light->color * triangle->
				specular_color *
				powf(std::max(dot(ray.direction, specular_direction), 0.0f),
...
{
	if (length(a.normal) == 0.f && length(b.normal) == 0 && length(c.normal) == 0)
		return geo_normal;
	return barycentric.x * a.normal + barycentric.y * b.normal * c.normal;
}
